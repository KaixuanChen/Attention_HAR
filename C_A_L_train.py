...
if isTrain:
    ...
    start = np.where(file_data[:, -1] == test_subject)[0][0]
    end = np.where(file_data[:, -1] == test_subject)[0][-1] + 1
    training_data = np.append(file_data[: start], file_data[end:], axis=0)
    ...
    training_data = PAMAP_Image(training_data)
...
training_label = training_data[:, -2]
